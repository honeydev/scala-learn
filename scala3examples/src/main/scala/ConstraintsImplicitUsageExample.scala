...
class AdoptionCenterService(adoptionServices: Seq[AdoptionCenter[?]]) {
  ...
  def testF[A](value: A =:= String) = {
    println(value)
  }
}

//case class AnotherCase[A, B](a: A, b: B) {
//  (a, b) match {
//    case (a: String, b: String) => Some(implicitly[A =:= B])
//    case _=> None
//  }
//}
//
object ConstraintsImplicitUsageExample extends App {
  val sdac = new DogAdoptionCenter()
  val scac = new CatAdoptionCenter()
  val acs = new AdoptionCenterService(Seq(sdac, scac))

  acs.testF[String]("abc")

  acs.adoptAll(Dog("ushi"), Cat("hvost"))
  ...
  val constraint = compareAnimalType(DogType, CatType)
